...
void TreePrune(TString infile=""){
  ...
  // If no input file provide as argument, prompt for one
  if(infile == ""){
  ...
  // Activate only the branches we want to keep, add more (or remove) as needed via the line below
  ...
  full_tree->SetBranchStatus("ReconstructedChargedParticleAssociations*",1);
  full_tree->SetBranchStatus("HcalFarForwardZDCClusters*",1);
  full_tree->SetBranchStatus("HcalFarForwardZDCClusterAssociations*",1);
  full_tree->SetBranchStatus("ReconstructedFarForwardZDC*",1);
