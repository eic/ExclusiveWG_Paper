...
A Rosenbluth separation involves the absolute subtraction of two measurements determined at high- and low-virtual photon polarization ($\epsilon_{Hi}$, $\epsilon_{Lo}$), corresponding to high and low electron beam energies, with very different detector rates. The resulting errors on $\sigma_L$ and $\sigma_T$ are magnified by $1/\Delta\epsilon=(\epsilon_{Hi}-\epsilon_{Lo})^{-1}$, i.e. there is no error cancellation. To keep the uncertainties in $\sigma_L$ to an acceptable level, $\Delta\epsilon >0.2$ is typically required, {\it i.e.} an uncertainty magnification of no more than 500\%. The measurements require continuous, high intensity electron beams, and detectors with good particle identification and reproducible systematics. JLab Hall~C is currently the only facility worldwide capable of such studies. 
...
Note that due to an overlap between the barrel and backwards end cap electromagnetic calorimeters, there is a region of inefficiency for reconstructing $e'$ clusters approximately between $160^{\circ} < \theta < 163^{\circ}$. If no calorimeter cluster is found matching the initial criteria, but a track that passes the selection cuts above is identified with $\theta$ in this range, the track information is assigned to the $e'$ for this event still. This is to compensate for inefficiency of in the calorimeter coverage in this overlap region.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.475\textwidth]{Figures/10on130_Q2_Comp_2D.png}
    \includegraphics[width=0.475\textwidth]{Figures/10on250_Q2_Comp_2D.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.475\textwidth]{Figures/10on130_t_Comp_2D.png}
    \includegraphics[width=0.475\textwidth]{Figures/10on250_t_Comp_2D.png}
...
Once calculated, a cut is applied on $-t$. Any events with\newline $0~<~-t_{eXBABE}~<$~1.4~GeV$^2$ are removed. A cut on $W~>~0$~GeV is also applied with any events failing this selection being removed. These cuts are applied simultaneously in conjunction with other cuts that are described in Sec.~\ref{subsec:Exclusivity_Cuts}.
...
\begin{gather*}
	\vec{P}_{M\_DEMP}.M2() < -1 \left(GeV/c^{2}\right)^{2},
	\vec{P}_{M\_DEMP}.M() > 0.75 GeV/c^{2},
\end{gather*}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.475\textwidth]{Figures/10on130_MissMass.png}
    \includegraphics[width=0.475\textwidth]{Figures/10on250_MissMass.png}
    \caption{The $\vec{P}_{M\_DEMP}.M()$ distribution for $10\times 130$ (left) and $10\times250$ (right) events. Events with $\vec{P}_{M\_DEMP}.M() > 0.75 GeV/c^{2}$, anything to the right of the dashed red line in each figure, are removed.}
\label{fig:MCut}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.475\textwidth]{Figures/10on130_MissMass2.png}
    \includegraphics[width=0.475\textwidth]{Figures/10on250_MissMass2.png}
    \caption{The $\vec{P}_{M\_DEMP}.M2()$ distribution for $10\times 130$ (left) and $10\times250$ (right) events. Events with $\vec{P}_{M\_DEMP}.M2() < -1 \left(GeV/c^{2}\right)^{2}$, anything to the left of the dashed red line in each figure, are removed.}
\label{fig:M2Cut}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.475\textwidth]{Figures/10on130_tResult_Q2_5_6.png}
    \includegraphics[width=0.475\textwidth]{Figures/10on250_tResult_Q2_5_6.png}
    \caption{The rates per $0.02~GeV^{2}$ -t bin for $10\times 130$ (left) and $10\times250$ (right) events in the $5~GeV/c^{2}< Q^{2} < 6~GeV/c^{2}$ bin.}
\label{fig:tQ2_Bins}
\end{figure}

It is vital that the $(Q^2,t)$-binning be at least as wide as the $t$ reconstruction resolutions shown in Figs. \ref{fig:Q2DA_Comp}, \ref{fig:teXBABE_Comp}, as significant bin migration will significantly hamper the theoretical interpretation of the results. This is unlikely to be an issue for $Q^{2}$ as the resolution varies from $\sim$0.02 to 0.35$~GeV/c^{2}$ across the $5-35$~$GeV/c^{2}$ range considered for $10\times130$ and $10\times250$ events as shown in Figs.~\ref{fig:Q2_Res_10x130},~\ref{fig:Q2_Res_10x250} respectively. This is comfortably within the $1~GeV/c^{2}$ width of $Q^{2}$ bins that was chosen.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.475\textwidth]{Figures/10on130_Q2Res_2D_AllQ2.png}
    \includegraphics[width=0.475\textwidth]{Figures/10on130_Q2Res_AllQ2.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.475\textwidth]{Figures/10on250_Q2Res_2D_AllQ2.png}
    \includegraphics[width=0.475\textwidth]{Figures/10on250_Q2Res_AllQ2.png}
...
For $-t$, the situation is slightly more complex. The $-t$ resolution ranges from $0.002--0.02~GeV^{2}$ for $10\times130$ in the $-t < 0.2~GeV^{2}$ range. For $10\times250$ , the corresponding resolution range is $0.01--0.025~GeV^{2}$ in the same range in $-t$. In both cases, the resolution increases with $-t$ as shown in Figs.~\ref{fig:t_Res_10x130},~\ref{fig:t_Res_10x250}. For the lowest 5 $-t$ bins that are used in the extraction of $F_{\pi}$, the resolution is smaller than the chosen $0.02~GeV^{2}$ bin width in the majority of cases. This situation also varies by $Q^{2}$ bin as discussed in Appendix.\ref{Appendix:tRes}. In higher $Q^{2}$ bins and for higher beam energy combinations, broader $-t$ bins may be needed in order to avoid a large bin migration correction as was the case at HERA.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.475\textwidth]{Figures/10on130_tRes_2D_AllQ2.png}
    \includegraphics[width=0.475\textwidth]{Figures/10on130_tRes_AllQ2.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.475\textwidth]{Figures/10on250_tRes_2D_AllQ2.png}
    \includegraphics[width=0.475\textwidth]{Figures/10on250_tRes_AllQ2.png}
    \caption{(Left) $\Delta t = t_{mc} - t_{eXBABE}$ as a function of $-t_{MC}$ for $10\times250$ events across all $Q^{2}_{DA}$ bins. One bin wide slices along the $x$-axis are taken from this figure. The RMS widths of these slices are plotted as a function of the bin centroid in $-t_{MC}$ in the (Right) figure. The $y$-axis can be interpreted as the resolution in $-t$.}
\label{fig:t_Res_10x250}
\end{figure}

To maximize statistics, events were integrated over the full $W$ range covered by the data in the $(Q^2,t)$ bin. We have not yet tested whether additional binning in $W$ will be needed to constrain the additional theoretical uncertainties discussed in Sec. \ref{sec:sigL_isolation}.
...
\begin{table}
    ...
    \multicolumn{6}{c}{$\overline{Q^2}$=10.54 GeV$^2$, $\overline{W}$=8.76 GeV, $-t_{min}$=0.02 GeV$^2$}\\
    \hline
    $-t_{bin}$ & $r=T/L$ & $\frac{Counts}{t-\phi\ bin}$ & $\delta\sigma_{uns}$ & $\delta\sigma_{L\ (stat+uncorr)}$ & $\delta\sigma_{L\ (t-corr+scale)}$\\
    \hline
     0.03 &  7.5\% & 2180 & 3.29\% & 3.55\% & 13.88\%\\
     0.05 &  8.8\% & 2710 & 3.15\% & 3.55\% & 14.45\%\\
     0.07 & 10.0\% & 2380 & 3.23\% & 3.77\% & 15.06\%\\
     0.09 & 12.6\% & 2180 & 3.29\% & 4.18\% & 16.41\%\\
     0.11 & 15.2\% & 1520 & 3.58\% & 4.85\% & 17.84\%\\ \hline
     \multicolumn{6}{|c|}{\small{$\delta F_{\pi}$ from 5 lowest $t$-bins: 1.74\% (stat+uncorr), 7.37\% (incl t-corr+scale)}} \\ 
     \hline
    \multicolumn{6}{c}{}\\
    \multicolumn{6}{c}{$\overline{Q^2}$=26.49 GeV$^2$, $\overline{W}$=9.40 GeV, $-t_{min}$=0.07 GeV$^2$}\\
    \hline
    $-t_{bin}$ & $r=T/L$ & $\frac{Counts}{t-\phi\ bin}$ & $\delta\sigma_{uns}$ & $\delta\sigma_{L\ (stat+uncorr)}$ & $\delta\sigma_{L\ (t-corr+scale)}$\\
    \hline
     0.09 &  3.5\% &  356 & 5.86\% & 5.79\% & 12.47\%\\
     0.11 &  4.4\% &  355 & 5.87\% & 5.82\% & 12.73\%\\
     0.13 &  5.3\% &  327 & 6.07\% & 6.03\% & 13.03\%\\
     0.15 &  6.4\% &  279 & 6.49\% & 6.48\% & 13.44\%\\
     0.17 &  7.5\% &  231 & 7.03\% & 7.05\% & 13.90\%\\ \hline
     \multicolumn{6}{|c|}{\small{$\delta F_{\pi}$ from 5 lowest $t$-bins: 2.77\% (stat+uncorr), 7.41\% (incl $t$-corr+scale)}} \\ \hline
